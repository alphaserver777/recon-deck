...
import { buildDashboardData } from "@/lib/ops-views/dashboard-vm";
import { DashboardClient } from "@/components/ops/dashboard/DashboardClient";
...
export default async function DashboardPage({ params }: PageProps) {
  ...
  const mapHosts = eng.hosts.map((h) => {
    ...
    const defenses: MapDefense[] = allDefenses
      .filter((d) => d.host_id === h.id || d.host_id === null)
      .map((d) => ({
        id: d.id,
        category: d.category,
        product: d.product,
  ...
  const dashData = buildDashboardData(sorted, commands, domain);
